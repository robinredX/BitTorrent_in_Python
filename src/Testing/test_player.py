...
class PlayerConnectionManager(object):
    ...
    def txt_reader(self):
        ...
        def read_text():
            logger.debug('Waiting for text input')
            while True:
                text = input('Enter message to send: ')
                print('+++++++servers: ', len(self.servers_queues))
                print('+++++++clients: ', len(self.clients_queues))
                if text.startswith('client'):
                    for q in self.clients_queues:
                        q.put(text[7:])
                elif text.startswith('server'):
                    for q in self.servers_queues:
                        q.put(text[7:])
                else:
                    print('Invalid start of message. Should start with "client" or "server"')
    ...
    def player_waiting_connection(self):
        ...
        def client():
            ...
            while True:
                print('waiting for connections: ', self.server_socket)
                client_socket, addr = self.server_socket.accept()
                logger.debug("Received connection", client_socket)

                self.servers_queues.append(queue.Queue())
        ...
        def queue_consumer():
            while True:
                ...
                if q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_REGISTER_HUB_QUEUE.value:
                    ...
                    if self.role != PLAYER_ROLE_SERVER_ONLY:
                        list_player = info
                        logger.debug(self.extra_log,
                                     'Manager receive queue message to connect players ' + str(list_player))
                        ...
                        if len(list_player):
                            ...
                            for player in list_player:
                                ...
                                if self.check_is_player_fraud(player):
                                    logger.error(self.extra_log,
                                                 'Mismatch between player_id and ip: reject player ' + str(player))
                                    fraud_player = self.client_player_dict[player['player_id']]['player_obj']
                                    fraud_player.kill_player()

                                else:
                                    self.client_player_dict[player['player_id']] = {'bitfield': None,
                                                                                    'status': PLAYER_STATUS_CHOKE,
                                                                                    'book_request': None,
                                                                                    'downloaded': 0, 'delay': []}
                                    ...
                                    self.clients_queues.append(queue.Queue())

                                    print('Connecting: ', player['ip'], player['port'])
                                    ...
                                    new_player = PlayerCommunicationClient(self.player_id,
                                                                           ...
                                                                           self.book,
                                                                           30,
                                                                           self.clients_queues[-1])
                                    logger.info(self.extra_log,
                                                '#### Connected to new client player ' + str(player['player_id']))
                                    ...
                                    # except:
                                    #     logger.warning(self.extra_log, 'Player cannot connect to player server ' + str(
                                    #         player['player_id']))
                    ...
                    if self.get_server_not_choke_client_number() < MAX_SERVER_CONNECTION_NUMBER and self.get_server_choke_client_number():
                        ...
                        if self.server_thread_timer != None:
                            if self.server_thread_timer.is_alive():
                                ...
                                logger.debug(self.extra_log,
                                             'Cancel server manger timer ' + str(self.server_thread_timer.is_alive()))
                    ...
                    if client_player_id in self.client_player_dict.keys():
                        ...
                        logger.debug(self.extra_log, 'Client ' + str(client_player_id) + ' status : ' + str(
                            bin(self.client_player_dict[client_player_id]['status'])))

                elif q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_UNCHOKE_CLIENT.value:
                    client_player_id = info
                    if client_player_id in self.client_player_dict.keys():
                        self.client_player_dict[client_player_id]['status'] &= ~PLAYER_STATUS_CHOKE
                        logger.info(self.extra_log, 'Client ' + str(client_player_id) + ' status : ' + str(
                            bin(self.client_player_dict[client_player_id]['status'])))
                    ...
                    if client_player_id in self.client_player_dict.keys():
                        self.client_player_dict[client_player_id]['bitfield'] = self.add_book_index(
                            self.client_player_dict[client_player_id]['bitfield'], book_index)
                        ...
                        logger.warning(self.extra_log, 'Player manager cannot find the player ' + str(
                            client_player_id) + ' to update bitfield')

                elif q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_BOOK_RECEIVED.value:
                    client_player_id, book_index, data_length, delay = info
                    logger.info(self.extra_log,
                                'Manager receive confirmation of book ' + str(book_index) + ' from player ' + str(
                                    client_player_id))
                    if client_player_id in self.client_player_dict.keys():
                    ...
                    logger.info(self.extra_log,
                                'Player manager receive notification a client player has been killed with id = ' + str(
                                    kill_player_id))
                    ...
                    for i in range(0, nb_client_player):
                        if self.server_player_list[i] == player:
                            logger.debug(self.extra_log, 'Player manager remove client ' + str(
                                kill_player_id) + 'client thread alive = ' + str(
                                self.server_player_list[i].is_client_alive()))
    ...
    def make_interested(self, player_id, send_notify):  # Whether to send interested message or not
        ...
        if player_id in self.client_player_dict.keys():
            if self.client_player_dict[player_id]['status'] & PLAYER_STATUS_CONNECTED:
                self.client_player_dict[player_id]['missing_book'] = self.book.match_bitfield(
                    self.client_player_dict[player_id]['bitfield'])
                if self.client_player_dict[player_id]['missing_book'] != []:
                    if not (self.client_player_dict[player_id]['status'] & PLAYER_STATUS_INTERESTED) or (
                            send_notify == True):
                    ...
                    if (self.client_player_dict[player_id]['status'] & PLAYER_STATUS_INTERESTED) or (
                            send_notify == True):
    ...
    def manage_server_client(self):
        ...
        if nb_i <= MAX_SERVER_CONNECTION_NUMBER:
            ...
            for player_id in self.server_player_dict.keys():
                downloaded_size = self.get_player_dowload_size(player_id)
                logger.info(self.extra_log, 'S- client ' + str(player_id) + ' status ' + str(
                    bin(self.server_player_dict[player_id]['status'])) \
                            + ' \t download ' + str(downloaded_size) + '\t upload ' + str(
                    self.server_player_dict[player_id]['uploaded']))
    ...
    def get_unchoke_client_player(self):
        player = []
        for player_id in self.client_player_dict.keys():
            if (self.client_player_dict[player_id]['status'] & PLAYER_STATUS_CONNECTED) and not (
                    self.client_player_dict[player_id]['status'] & PLAYER_STATUS_CHOKE):
                player.append(player_id)
        return player
        ...
        while nb_active_players < 4 and nb_standby_players:
            ...
            if len(freq_key):
                ...
                while player_found == False:
                    ...
                    logger.debug(self.extra_log,
                                 'Candidate book with freq=' + str(frequency) + ' ' + str(self.candidate_book_index))
                            ...
                            if self.check_book_not_requested(book_index) == True:
                                ...
                                for player_id in self.client_player_dict.keys():
                                    if (self.client_player_dict[player_id]['status'] & PLAYER_STATUS_CONNECTED) \
                                            ...
                                            and not (
                                            self.client_player_dict[player_id]['status'] & PLAYER_STATUS_ACTIVE):
                                        if book_index in self.client_player_dict[player_id]['missing_book']:
                                            list_book_owner.append(player_id)
                                            logger.info(self.extra_log,
                                                        'C- Player ' + str(player_id) + ' can accept index ' + str(
                                                            book_index) + ' book request ')
                ...
                if len(list_book_owner):
                    ...
                    logger.info(self.extra_log, 'C- Manager send request book ' + str(book_index) + ' to player ' + str(
                        picked_player_id))
                    if self.client_player_dict[picked_player_id]['player_obj'] != None:  # to make it run in test mode
                        self.client_player_dict[picked_player_id]['player_obj'].get_client_queue().put(
                            (PlayerQMsgEnum.QUEUE_MSG_REQUEST, book_index))
            ...
            nb_active_players = len(list_active)
            nb_standby_players = len(list_standby)
            logger.debug('C- There are ' + str(nb_active_players) + ' active players' + ' and ' + str(
                nb_standby_players) + ' standby player')
            loop += 1
...
class PlayerCommunicationClient(object):
    ...
    def __init__(self, player_id, client_ip, client_listening_port, client_player_id, meta_file, manager_queue, book,
                 max_connections, txt_queue):  # Maximum number of connections
        ...
        self.t3 = self.text_queue_consumer()
        self.t3.daemon = True
        self.t3.start()
    ...
    def text_queue_consumer(self):
        def consumer():
            msg_send = b''
            while True:

                text = self.txt_queue.get()
                print('-------------------client thread: ', text)

                if text.startswith('bitfield'):
                    if 'rand' in text:
                        bytes_size = int(math.ceil(self.meta_file.get_book_number() / 8))
                        bitfield = bytearray(bytes_size)
                    else:
                        bitfield = self.book.get_bitfield()

                    msg_send = message.BitfieldMsg(bitfield).msg_encode()

                elif text.startswith('have'):
                    if 'rand' in text:
                        book_index = random.randint(0, self.meta_file.get_book_number())
                    else:
                        book_index = int(text[5:].strip())
                    msg_send = message.HaveMsg(book_index).msg_encode()

                elif text.startswith('choke'):
                    msg_send = message.ChokeMsg().msg_encode()
                    # self.client_choke = True

                elif text.startswith('unchoke'):
                    msg_send = message.UnchokeMsg().msg_encode()
                    # self.client_choke = False

                elif text.startswith('book'):
                    if 'rand' in text:
                        rand_data = os.urandom(16384)
                        book_index = random.randint(0, self.meta_file.get_book_number())
                        msg_send = message.BookMsg(book_index, rand_data).msg_encode()
                    else:
                        book_index = int(text[5:].strip())
                        self.book.queue_read(book_index, (self.q, PlayerQMsgEnum.QUEUE_MSG_PAYLOAD_READ))

                elif text.startswith('handshake'):
                    msg_send = message.HandshakeMsg(self.player_id, self.meta_file.get_info_hash()).msg_encode()

                elif text.startswith('interested'):
                    msg_send = message.InterestedMsg().msg_encode()
                    # self.player_interested = True

                elif text.startswith('not_interested'):
                    msg_send = message.NotInterestedMsg().msg_encode()
                    ...
                    if 'rand' in text:
                        ...
                        book_index = int(text[5:].strip())
                    msg_send = message.RequestMsg(book_index).msg_encode()
            ...
            while True:
                ...
                if msg != b'':
                    ...
                    if status == 0:
                        ...
                        if msg_id == 'bitfield':
                            logger.info(self.extra_log,
                                        'C- Got bitfield message from player ' + str(self.client_player_id))
                            self.client_bitfield = objMsg.get_bitfield()
                            self.manager_q.put((PlayerQMsgEnum.QUEUE_MSG_BITFIELD_REGISTER,
                                                (self.client_player_id, self.client_bitfield)))
                            ...
                            if objMsg.get_book_index() == self.requested_book:
                                ...
                                if self.meta_file.get_book_hash(self.requested_book) == sha1(payload).digest():
                                    ...
                                    logger.info(self.extra_log,
                                                'Book received in ' + str(self.reception_delay) + ' from player ' + str(
                                                    self.client_player_id))

                                    self.book.queue_write(self.requested_book, payload,
                                                          (self.q, PlayerQMsgEnum.QUEUE_MSG_PAYLOAD_WRITE))
            ...
            while True:
                if state == STATE_INIT:
                    ...
                    if q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_REQUEST.value:
                        logger.debug(self.extra_log, 'Receive send request for player ' + str(
                            self.client_player_id) + ' and server_ckoke=' + str(self.client_choke) + ' at t=' + str(
                            time.time()))
                        ...
                        if self.client_choke == False:
                            ...
                            if self.requested_book is not None:
                                logger.debug(self.extra_log,
                                             'A request from manager has been received for client ' + str(
                                                 self.client_player_id))
                        ...
                        self.manager_q.put((PlayerQMsgEnum.QUEUE_MSG_BOOK_RECEIVED,
                                            (self.client_player_id, book_index, data_length, self.reception_delay)))
                        ...
                        self.manager_q.put(
                            (PlayerQMsgEnum.QUEUE_MSG_KILL_CNX_PLAYER_CLIENT, (self.client_player_id, self.client_ip)))
                        state = STATE_PLAYER_KILLED
                        break
...
class PlayerCommunicationServer(object):
    ...
    def __init__(self, player_id, book, client_socket, addr, meta_file, manager_queue,
                 max_connections, txt_queue):  # Maximum number of connections
        self.addr = addr
                ...
                if msg != b'':
                    ...
                    if status == 0:
                        ...
                        if msg_id == 'handshake':  # Received handshake, put in queue to send bitfield
                            ...
                            logger.info(self.extra_log, 'Receive handshake message from player ' + str(
                                self.client_player_id) + ' with adr ' + str(self.addr))
                            self.q.put((PlayerQMsgEnum.QUEUE_MSG_BITFIELD, None))
                            self.manager_q.put(
                                (PlayerQMsgEnum.QUEUE_MSG_HANDSHAKE_SERVER, (self, self.client_player_id)))

                        elif msg_id == 'interested':  # Received interested message, put in queue of inetersted clients
                            logger.debug(self.extra_log,
                                         'Receive interested message from player ' + str(self.client_player_id))
                            ...
                            logger.debug(self.extra_log,
                                         'Receive not interested message from player ' + str(self.client_player_id))
                            ...
                            book_index = objMsg.get_book_index()
                            logger.info(self.extra_log,
                                        'Receive request message book ' + str(book_index) + ' for player ' + str(
                                            self.client_player_id) +
                                        ' with interest=' + str(self.client_interested) + ' choke=' + str(
                                            self.client_choke) + ' at t=' + str(time.time()))
                            if self.client_interested == True and self.client_choke == False:
                                ...
                                logger.debug(self.extra_log,
                                             'Book request received but the client was not interested or choked')  # TODO How to deal with this? Can this happen?
            ...
            if state == STATE_WAIT_QUEUE:
                while True:
                    ...
                    if q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_BITFIELD.value:
                        ...
                        logger.debug(self.extra_log,
                                     'Send choke to player ' + str(self.client_player_id) + ' at t=' + str(time.time()))
                        msg_send = message.ChokeMsg().msg_encode()
                        self.client_choke = True

                    elif q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_UNCHOKE_SERVER.value:  # Send unchoke message
                        logger.debug(self.extra_log,
                                     'Send unchoke to player ' + str(self.client_player_id) + ' at t=' + str(
                                         time.time()))
                        msg_send = message.UnchokeMsg().msg_encode()
                        self.client_choke = False
                        ...
                        logger.debug(self.extra_log,
                                     'Received payload from book manager for player ' + str(self.client_player_id))
                        book_index, payload = info
                        msg_send = message.BookMsg(book_index, payload).msg_encode()
                        # notify manager to update 'uploaded'
                        self.manager_q.put(
                            (PlayerQMsgEnum.QUEUE_MSG_CLIENT_UPLOAD, (self.client_player_id, len(payload))))

                    elif q_msg_id.value == PlayerQMsgEnum.QUEUE_MSG_KILL_CONNECTION.value:
                        logger.warning(self.extra_log, 'Killing sending server player ' + str(self.client_player_id))
                        self.manager_q.put(
                            (PlayerQMsgEnum.QUEUE_MSG_KILL_CNX_PLAYER_SERVER, (self, self.client_player_id, self.addr)))
                        state = STATE_PLAYER_KILLED
                        break
